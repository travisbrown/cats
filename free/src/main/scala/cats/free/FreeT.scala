...
object FreeT extends FreeTInstances {
  ...
  def compile[S[_], T[_], M[_]: Functor](
    st: FunctionK[S, T]
  ): FunctionK[({ type λ[α$] = FreeT[S, M, α$] })#λ, ({ type λ[α$] = FreeT[T, M, α$] })#λ] =
    new FunctionK[({ type λ[α$] = FreeT[S, M, α$] })#λ, ({ type λ[α$] = FreeT[T, M, α$] })#λ] {
      def apply[A$](f: FreeT[S, M, A$]): FreeT[T, M, A$] = f.compile(st)
    }

  def foldMap[S[_], M[_]: Monad](fk: FunctionK[S, M]): FunctionK[({ type λ[α$] = FreeT[S, M, α$] })#λ, M] =
    new FunctionK[({ type λ[α$] = FreeT[S, M, α$] })#λ, M] { def apply[A$](f: FreeT[S, M, A$]): M[A$] = f.foldMap(fk) }
...
sealed abstract private[free] class FreeTInstances extends FreeTInstances0 {
  implicit def catsFreeMonadErrorForFreeT[S[_], M[_], E](
    implicit E: MonadError[M, E]
  ): MonadError[({ type λ[α$] = FreeT[S, M, α$] })#λ, E] =
    new MonadError[({ type λ[α$] = FreeT[S, M, α$] })#λ, E] with FreeTMonad[S, M] {
      override def M = E
...
sealed abstract private[free] class FreeTInstances0 extends FreeTInstances1 {
  implicit def catsFreeMonadForFreeT[S[_], M[_]](
    implicit M0: Applicative[M]
  ): Monad[({ type λ[α$] = FreeT[S, M, α$] })#λ] =
    new FreeTMonad[S, M] {
      def M = M0
    }
}
...
sealed abstract private[free] class FreeTInstances1 extends FreeTInstances2 {
  implicit def catsFreeFlatMapForFreeT[S[_], M[_]](
    implicit M0: Applicative[M]
  ): FlatMap[({ type λ[α$] = FreeT[S, M, α$] })#λ] =
...
sealed abstract private[free] class FreeTInstances2 extends FreeTInstances3 {
  implicit def catsFreeAlternativeForFreeT[S[_], M[_]: Alternative: Monad]
    : Alternative[({ type λ[α$] = FreeT[S, M, α$] })#λ] =
    new Alternative[({ type λ[α$] = FreeT[S, M, α$] })#λ] with FreeTMonad[S, M] with FreeTMonoidK[S, M] {
...
sealed abstract private[free] class FreeTInstances3 {
  implicit def catsFreeSemigroupKForFreeT[S[_], M[_]: Applicative: SemigroupK]
    : SemigroupK[({ type λ[α$] = FreeT[S, M, α$] })#λ] =
...
sealed private[free] trait FreeTFlatMap[S[_], M[_]] extends FlatMap[({ type λ[α$] = FreeT[S, M, α$] })#λ] {
...
sealed private[free] trait FreeTMonad[S[_], M[_]]
    extends Monad[({ type λ[α$] = FreeT[S, M, α$] })#λ]
    with FreeTFlatMap[S, M] {
  implicit def M: Applicative[M]
...
sealed private[free] trait FreeTMonoidK[S[_], M[_]]
    extends MonoidK[({ type λ[α$] = FreeT[S, M, α$] })#λ]
    with FreeTSemigroupK[S, M] {
  implicit def M: Applicative[M]
  def M1: MonoidK[M]
  final override def empty[A]: FreeT[S, M, A] = FreeT.liftT[S, M, A](M1.empty[A])(M)
}

sealed private[free] trait FreeTSemigroupK[S[_], M[_]] extends SemigroupK[({ type λ[α$] = FreeT[S, M, α$] })#λ] {
  implicit def M: Applicative[M]
  def M1: SemigroupK[M]
