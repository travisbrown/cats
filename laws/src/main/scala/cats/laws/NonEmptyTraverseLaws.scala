...
import cats.{Apply, Id, NonEmptyTraverse, Semigroup, catsInstancesForId}
...
trait NonEmptyTraverseLaws[F[_]] extends TraverseLaws[F] with ReducibleLaws[F] {
  ...
  def nonEmptyTraverseSequentialComposition[A, B, C, M[_], N[_]](
    ...
    val rhs = fa.nonEmptyTraverse[({ type λ[α$] = Nested[M, N, α$] })#λ, C](a => Nested(M.map(f(a))(g)))
    lhs <-> rhs
  }
  ...
  def reduceMapDerived[A, B](
    ...
    val lhs: B = fa.nonEmptyTraverse[({ type λ[α$] = Const[B, α$] })#λ, B](a => Const(f(a))).getConst
