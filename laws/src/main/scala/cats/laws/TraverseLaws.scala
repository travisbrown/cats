...
trait TraverseLaws[F[_]] extends FunctorLaws[F] with FoldableLaws[F] with UnorderedTraverseLaws[F] {
  ...
  def traverseSequentialComposition[A, B, C, M[_], N[_]](
    ...
    val rhs = fa.traverse[({ type λ[α$] = Nested[M, N, α$] })#λ, C](a => Nested(M.map(f(a))(g)))
    lhs <-> rhs
  }
  ...
  def foldMapDerived[A, B](
    ...
    val lhs: B = fa.traverse[({ type λ[α$] = Const[B, α$] })#λ, B](a => Const(f(a))).getConst
  ...
  def traverseOrderConsistent[A](fa: F[A]): IsEq[Option[A]] = {
    ...
    val first = F.traverse[({ type λ[α$] = Const[FirstOption[A], α$] })#λ, A, A](fa)(store).getConst.o
    val traverseFirst = F
      .traverse[({ type λ[α$] = Const[FirstOption[A], α$] })#λ, A, A](
