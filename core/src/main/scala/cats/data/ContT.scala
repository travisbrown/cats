...
object ContT {
  ...
  implicit def catsDataContTDefer[M[_], B]: Defer[({ type λ[α$] = ContT[M, B, α$] })#λ] =
    new Defer[({ type λ[α$] = ContT[M, B, α$] })#λ] {
      def defer[A](c: => ContT[M, B, A]): ContT[M, B, A] =
        DeferCont(() => c)
    }

  implicit def catsDataContTMonad[M[_]: Defer, A]: Monad[({ type λ[α$] = ContT[M, A, α$] })#λ] =
    new Monad[({ type λ[α$] = ContT[M, A, α$] })#λ] {
