...
sealed private[data] trait IdTFunctor[F[_]] extends Functor[({ type λ[α$] = IdT[F, α$] })#λ] {
...
sealed private[data] trait IdTApply[F[_]] extends Apply[({ type λ[α$] = IdT[F, α$] })#λ] with IdTFunctor[F] {
...
sealed private[data] trait IdTApplicative[F[_]] extends Applicative[({ type λ[α$] = IdT[F, α$] })#λ] with IdTApply[F] {
  implicit val F0: Applicative[F]

  def pure[A](a: A): IdT[F, A] = IdT.pure(a)
}

sealed private[data] trait IdTContravariantMonoidal[F[_]]
    extends ContravariantMonoidal[({ type λ[α$] = IdT[F, α$] })#λ] {
...
sealed private[data] trait IdTFlatMap[F[_]] extends FlatMap[({ type λ[α$] = IdT[F, α$] })#λ] with IdTApply[F] {
...
sealed private[data] trait IdTMonad[F[_]]
    extends Monad[({ type λ[α$] = IdT[F, α$] })#λ]
    with IdTApplicative[F]
    with IdTFlatMap[F] {
  implicit val F0: Monad[F]
}

sealed private[data] trait IdTFoldable[F[_]] extends Foldable[({ type λ[α$] = IdT[F, α$] })#λ] {
...
sealed private[data] trait IdTTraverse[F[_]]
    extends Traverse[({ type λ[α$] = IdT[F, α$] })#λ]
    with IdTFoldable[F]
    with IdTFunctor[F] {
...
sealed private[data] trait IdTNonEmptyTraverse[F[_]]
    ...
    with NonEmptyTraverse[({ type λ[α$] = IdT[F, α$] })#λ]
    with IdTFunctor[F] {
...
sealed abstract private[data] class IdTInstances8 {
  ...
  ): CommutativeFlatMap[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTFlatMap[F] with CommutativeFlatMap[({ type λ[α$] = IdT[F, α$] })#λ] {
      implicit val F0: CommutativeFlatMap[F] = F
    }
}

sealed abstract private[data] class IdTInstances7 extends IdTInstances8 {
  implicit def catsDataCommutativeMonadForIdT[F[_]](
    implicit F: CommutativeMonad[F]
  ): CommutativeMonad[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTMonad[F] with CommutativeMonad[({ type λ[α$] = IdT[F, α$] })#λ] { implicit val F0: CommutativeMonad[F] = F }
}
...
sealed abstract private[data] class IdTInstances6 extends IdTInstances7 {
  ...
  ): ContravariantMonoidal[({ type λ[α$] = IdT[F, α$] })#λ] =
...
sealed abstract private[data] class IdTInstances5 extends IdTInstances6 {
  implicit def catsDataFunctorForIdT[F[_]](implicit F: Functor[F]): Functor[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTFunctor[F] { implicit val F0: Functor[F] = F }
}

sealed abstract private[data] class IdTInstances4 extends IdTInstances5 {
  implicit def catsDataApplyForIdT[F[_]](implicit F: Apply[F]): Apply[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTApply[F] { implicit val F0: Apply[F] = F }
}

sealed abstract private[data] class IdTInstances3 extends IdTInstances4 {
  implicit def catsDataApplicativeForIdT[F[_]](
    implicit F: Applicative[F]
  ): Applicative[({ type λ[α$] = IdT[F, α$] })#λ] =
...
sealed abstract private[data] class IdTInstances2 extends IdTInstances3 {
  implicit def catsDataFlatMapForIdT[F[_]](implicit F: FlatMap[F]): FlatMap[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTFlatMap[F] { implicit val F0: FlatMap[F] = F }
}

sealed abstract private[data] class IdTInstances1 extends IdTInstances2 {
  implicit def catsDataMonadForIdT[F[_]](implicit F: Monad[F]): Monad[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTMonad[F] { implicit val F0: Monad[F] = F }

  implicit def catsDataFoldableForIdT[F[_]](implicit F: Foldable[F]): Foldable[({ type λ[α$] = IdT[F, α$] })#λ] =
    new IdTFoldable[F] { implicit val F0: Foldable[F] = F }
}

sealed abstract private[data] class IdTInstances0 extends IdTInstances1 {

  implicit def catsDataTraverseForIdT[F[_]](implicit F: Traverse[F]): Traverse[({ type λ[α$] = IdT[F, α$] })#λ] =
...
sealed abstract private[data] class IdTInstances extends IdTInstances0 {
  ...
  implicit def catsDataNonEmptyTraverseForIdT[F[_]](
    implicit F: NonEmptyTraverse[F]
  ): NonEmptyTraverse[({ type λ[α$] = IdT[F, α$] })#λ] =
