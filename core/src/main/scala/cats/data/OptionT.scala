...
object OptionT extends OptionTInstances {
   ...
   * }}}
   */
  def liftK[F[_]](implicit F: Functor[F]): F ~> ({ type λ[α$] = OptionT[F, α$] })#λ =
    new (F ~> ({ type λ[α$] = OptionT[F, α$] })#λ) { def apply[A$](a$ : F[A$]): OptionT[F, A$] = OptionT.liftF(a$) }
}

sealed abstract private[data] class OptionTInstances extends OptionTInstances0 {
  // to maintain binary compatibility
  def catsDataMonadForOptionT[F[_]](implicit F0: Monad[F]): Monad[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTMonad[F] { implicit val F = F0 }

  implicit def catsDataTraverseForOptionT[F[_]](
    implicit F0: Traverse[F]
  ): Traverse[({ type λ[α$] = OptionT[F, α$] })#λ] =
  ...
  implicit def catsDataDeferForOptionT[F[_]](implicit F: Defer[F]): Defer[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new Defer[({ type λ[α$] = OptionT[F, α$] })#λ] {
      def defer[A](fa: => OptionT[F, A]): OptionT[F, A] =
        OptionT(F.defer(fa.value))
    }

  implicit def catsDataTraverseFilterForOptionT[F[_]](
    implicit F0: Traverse[F]
  ): TraverseFilter[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTFunctorFilter[F] with TraverseFilter[({ type λ[α$] = OptionT[F, α$] })#λ] {
      implicit def F: Functor[F] = F0

      val traverse: Traverse[({ type λ[α$] = OptionT[F, α$] })#λ] = OptionT.catsDataTraverseForOptionT[F]
  ...
  def catsDateTraverseFilterForOptionT[F[_]](
    implicit F0: Traverse[F]
  ): TraverseFilter[({ type λ[α$] = OptionT[F, α$] })#λ] =
  ...
  implicit def catsDataParallelForOptionT[M[_]](
    ...
    new Parallel[({ type λ[α$] = OptionT[M, α$] })#λ] {
      type F[x] = Nested[P.F, Option, x]

      implicit val monadM: Monad[M] = P.monad

      def applicative: Applicative[({ type λ[α$] = Nested[P.F, Option, α$] })#λ] =
        cats.data.Nested.catsDataApplicativeForNested(P.applicative, cats.instances.option.catsStdInstancesForOption)

      def monad: Monad[({ type λ[α$] = OptionT[M, α$] })#λ] = cats.data.OptionT.catsDataMonadErrorMonadForOptionT[M]

      def sequential: ({ type λ[α$] = Nested[P.F, Option, α$] })#λ ~> ({ type λ[α$] = OptionT[M, α$] })#λ =
        new (({ type λ[α$] = Nested[P.F, Option, α$] })#λ ~> ({ type λ[α$] = OptionT[M, α$] })#λ) {
          def apply[A$](nested: Nested[P.F, Option, A$]): OptionT[M, A$] = OptionT(P.sequential(nested.value))
        }

      def parallel: ({ type λ[α$] = OptionT[M, α$] })#λ ~> ({ type λ[α$] = Nested[P.F, Option, α$] })#λ =
        new (({ type λ[α$] = OptionT[M, α$] })#λ ~> ({ type λ[α$] = Nested[P.F, Option, α$] })#λ) {
...
sealed abstract private[data] class OptionTInstances0 extends OptionTInstances1 {
  ...
  ): MonadError[({ type λ[α$] = OptionT[F, α$] })#λ, E] { type Dummy } =
  ...
  ): ContravariantMonoidal[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTContravariantMonoidal[F] { implicit val F = F0 }

  implicit def catsDataMonoidKForOptionT[F[_]](implicit F0: Monad[F]): MonoidK[({ type λ[α$] = OptionT[F, α$] })#λ] =
  ...
  implicit def catsDateFunctorFilterForOptionT[F[_]](
    implicit F0: Functor[F]
  ): FunctorFilter[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTFunctorFilter[F] { implicit val F = F0 }

  implicit def catsDataContravariantForOptionT[F[_]](
    implicit F0: Contravariant[F]
  ): Contravariant[({ type λ[α$] = OptionT[F, α$] })#λ] =
...
sealed abstract private[data] class OptionTInstances1 extends OptionTInstances2 {
  implicit def catsDataSemigroupKForOptionT[F[_]](
    implicit F0: Monad[F]
  ): SemigroupK[({ type λ[α$] = OptionT[F, α$] })#λ] =
  ...
  implicit def catsDataMonadErrorMonadForOptionT[F[_]](
    implicit F0: Monad[F]
  ): MonadError[({ type λ[α$] = OptionT[F, α$] })#λ, Unit] =
...
sealed abstract private[data] class OptionTInstances2 extends OptionTInstances3 {
  implicit def catsDataFoldableForOptionT[F[_]](
    implicit F0: Foldable[F]
  ): Foldable[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTFoldable[F] { implicit val F = F0 }

  implicit def catsDataInvariantForOptionT[F[_]](
    implicit F0: Invariant[F]
  ): Invariant[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTInvariant[F] { implicit val F = F0 }
}

sealed abstract private[data] class OptionTInstances3 {
  implicit def catsDataFunctorForOptionT[F[_]](implicit F0: Functor[F]): Functor[({ type λ[α$] = OptionT[F, α$] })#λ] =
    new OptionTFunctor[F] { implicit val F = F0 }
}

private[data] trait OptionTFunctor[F[_]] extends Functor[({ type λ[α$] = OptionT[F, α$] })#λ] {
  implicit def F: Functor[F]

  override def map[A, B](fa: OptionT[F, A])(f: A => B): OptionT[F, B] = fa.map(f)
}

sealed private[data] trait OptionTInvariant[F[_]] extends Invariant[({ type λ[α$] = OptionT[F, α$] })#λ] {
...
sealed private[data] trait OptionTContravariant[F[_]] extends Contravariant[({ type λ[α$] = OptionT[F, α$] })#λ] {
...
private[data] trait OptionTMonad[F[_]] extends Monad[({ type λ[α$] = OptionT[F, α$] })#λ] {
...
private[data] trait OptionTMonadErrorMonad[F[_]]
    extends MonadError[({ type λ[α$] = OptionT[F, α$] })#λ, Unit]
    with OptionTMonad[F] {
  implicit def F: Monad[F]
...
private trait OptionTMonadError[F[_], E]
    extends MonadError[({ type λ[α$] = OptionT[F, α$] })#λ, E]
    with OptionTMonad[F] {
...
private trait OptionTContravariantMonoidal[F[_]] extends ContravariantMonoidal[({ type λ[α$] = OptionT[F, α$] })#λ] {
...
private[data] trait OptionTFoldable[F[_]] extends Foldable[({ type λ[α$] = OptionT[F, α$] })#λ] {
...
sealed private[data] trait OptionTTraverse[F[_]]
    extends Traverse[({ type λ[α$] = OptionT[F, α$] })#λ]
...
private[data] trait OptionTSemigroupK[F[_]] extends SemigroupK[({ type λ[α$] = OptionT[F, α$] })#λ] {
  implicit def F: Monad[F]

  def combineK[A](x: OptionT[F, A], y: OptionT[F, A]): OptionT[F, A] = x.orElse(y)
}

private[data] trait OptionTMonoidK[F[_]]
    extends MonoidK[({ type λ[α$] = OptionT[F, α$] })#λ]
    with OptionTSemigroupK[F] {
...
sealed private[data] trait OptionTFunctorFilter[F[_]] extends FunctorFilter[({ type λ[α$] = OptionT[F, α$] })#λ] {
  implicit def F: Functor[F]

  def functor: Functor[({ type λ[α$] = OptionT[F, α$] })#λ] = OptionT.catsDataFunctorForOptionT[F]
