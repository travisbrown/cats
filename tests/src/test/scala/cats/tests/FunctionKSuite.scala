...
class FunctionKSuite extends CatsSuite {
  ...
  val Test1FK = new FunctionK[Test1Algebra, Id] { def apply[A$](a$ : Test1Algebra[A$]): Id[A$] = a$.v }
  val Test2FK = new FunctionK[Test2Algebra, Id] { def apply[A$](a$ : Test2Algebra[A$]): Id[A$] = a$.v }
